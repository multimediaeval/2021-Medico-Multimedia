...
Models will be evaluated based on the performance metrics used to evaluate the polyp segmentation task and the number of frames that can be segmented per second. Submission will be ranked based on a balanced metric between predictive performance and speed. All submissions are evaluated on what can be considered consumer-grade hardware, that is, a computer with an Intel Core i9-10900K processor, an Nvidia GeForce RTX 3090 graphics processing unit (GPU), and 32 gigabytes of RAM.

We require you to build a Docker image of your submission which includes all required dependencies and can be run using the latest version of Docker. Please note that the data should not be included within the Docker image itself, as it will be injected by us. Assume that the test dataset will be located at `/medico2021test` and the masks should be saved to `/medico2021mask`. An example submission is included within this repository (`sample-submission`), where we show an example of a Keras-based submission.

To test the submission, frist build the image using `docker build .`, then run it using the following command:

```bash
sudo docker run -v <test_set_location>:/medico2021test -v <output_location>:/medico2021mask -a stdin -a stdout -a stderr <docker_id> > medico_submission.txt
```
